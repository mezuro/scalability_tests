...
public class Update extends RESTStrategy {
	...
	public void beforeExperiment() throws Exception {
		...
		jsonBody = new JSONObject(parameters);
		response = post(buildUrl(METRIC_CONFIGURATION_PATH + "/" + metricConfigurationId + "/" + KALIBRO_RANGE_PATH), jsonBody);
	...
	public String request(String string) throws Exception {
		...
		parameters.put("beginning", "0.0");
		parameters.put("end", "1.0");
		
		JSONObject jsonBody = new JSONObject(parameters);
		put(buildUrl(METRIC_CONFIGURATION_PATH + "/" + metricConfigurationId + "/" + KALIBRO_RANGE_PATH + "/" + rangeId), jsonBody);
		return rangeId;
	}
